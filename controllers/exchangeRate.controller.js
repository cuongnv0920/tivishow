    ...
    request(url, (error, response, html) => {
      if (!error && response.statusCode === 200) {
        ...
        $("#table").each((index, el) => {
          ...
          const centerExchangeRate = $(el) // crawler tỷ giá trung tâm
            .find("td i")
            .text()
            .split(")")
            .slice(1);
          notifications.push(centerExchangeRate[0]);
        });
    ...
    await data[0]?.map((row) => {
      return ExchangeRate.updateMany(
        { currency: row.currency },
        {
          ...
          centerExchangeRate: Number(crawler[0].notifications[3]),
          updatedAt: Date.now(),
        }
      )
        .then(() => {
...
module.exports.getAll = async (req, res, next) => {
  ...
  await ExchangeRate.find({ $and: [{ softDelete: null }, { status: true }] })
    .skip(limit * page - limit)
    .limit(limit)
    .sort({ sort: 1 })
    .exec((error, exchangeRates) => {
      ExchangeRate.countDocuments((error, total) => {
